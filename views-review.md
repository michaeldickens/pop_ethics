...
unrankable verdict simply contributes no link. Ranks the same-number pair determinately
while leaving three different-number ones unrankable, which is Parfit's own shape: the
Same Number Quality Claim settles a same-number choice by how well the lives go, whoever
lives them. Should be told what that line owes, and not scored as a conflict.
...
- You judged 3 of the 8 pairs unrankable.
- You hold the Procreation Asymmetry.
...
Answers 'cannot be ranked' to every comparison and rejects every structural principle,
including the same-number pair where identity is the only thing that varies. A boundary
case: there is nothing to be inconsistent with, so the quiz should find no collisions,
and should not manufacture one. If this profile ever produces a conflict, something is
wrong. It should still be told, at length, how little such an ordering can do.
