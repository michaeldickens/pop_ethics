...
var AP_POP = [
  CHAIN[0].plus[0],
  { n: CHAIN[0].plus[1].n, w: CHAIN[0].plus[1].w, tag: "the added hundred" },
];
...
var K_BOTH = [
  { n: 499, w: 55 },
  { n: 1, w: 20, tag: "Owen", side: "left" },
  { n: 1, w: K_WOND[1].w, tag: "Nadia" },
];
...
var K_MOD_OWEN = [
  { n: 499, w: 55 },
  { n: 1, w: K_BASE[0].w, tag: "Owen", side: "left" },
  { n: 1, w: K_MOD[1].w, tag: "Nadia" },
];
...
var QUESTIONS = [
  {
    ...
    pops: [PARETO_BEFORE, PARETO_AFTER],
    names: ["Before", "After"],
    ...
    opts: [
      ["A", "Yes", "yes"],
      ["B", "No \u2013 they are equal or incomparable", "no"],
    ],
  },
  {
    id: "same_number",
    kind: "pair",
    label: "The same number, different people",
  ...
  },
  {
    id: "misery",
    kind: "pair",
    label: "Adding a life of suffering",
    pops: [K_BASE, K_BAD],
    names: ["K", "K\u2212"],
  ...
  },
  {
    id: "neutral_mod",
    kind: "pair",
    label: "Adding a modest good life",
    pops: [K_BASE, K_MOD],
    names: ["K", "K+"],
  ...
  },
  {
    ...
    body:
      "Repeat the two moves from the last two questions: (1) make everyone slightly better off and create a new group of less-happy people; then (2) keep the same population, but increase both total welfare and equality. Repeat again and again a total of " +
      CHAIN.length +
    ...
    opts: [
      ["A", "Yes \u2014 the same at every rung", "yes"],
      ["B", "No \u2014 somewhere my verdict flips", "no"],
    ],
    chain: true,
  },
  {
  ...
  },
  {
    id: "collapse",
    kind: "principle",
    label: "The collapsing principle",
    ...
    when: function (a) {
      return !!collapseCandidate(a);
    },
    title: "Where does \u201Cno fact of the matter\u201D end?",
    body: function (a) {
      var c = collapseCandidate(a);
      var lo = Math.min(c.vague.w, c.anchor.w),
        hi = Math.max(c.vague.w, c.anchor.w);
      return (
        "This question is here because of two answers you have already given. You said that adding Nadia with <strong>" +
        c.vague.life +
        "</strong> (wellbeing " +
        c.vague.w +
        ") <strong>cannot be ranked</strong> against leaving her out \u2014 not better, not worse, not exactly as good. But adding her with <strong>" +
        c.anchor.life +
        "</strong> (wellbeing " +
        c.anchor.w +
        ") you ranked determinately: that world is " +
        (c.dir === "up"
          ? "<strong>better</strong>"
          : "<strong>worse</strong>") +
        " than leaving her out. Somewhere between " +
        lo +
        " and " +
        hi +
        " for her, then, lies a boundary \u2014 a point where a hair\u2019s change to her life, one unit, a single good afternoon, turns \u201Cthere is no fact of the matter\u201D into a settled verdict."
      );
    },
    ask: "Can so small a change carry so much weight?",
    opts: [
      ["A", "No \u2014 so the earlier case was never really unrankable", "yes"],
  ...
  },
  {
    id: "greedy",
    kind: "pair",
    label: "A harm beside the addition",
    ...
    when: function (a) {
      return a.neutral_mod !== "right" || a.neutral_wond !== "right";
    },
    pops: [K_BASE_OWEN, K_BOTH],
    names: ["K", "K±"],
    title: "One person worse off, one person added.",
    body:
      "<strong>K</strong> once more, and beside it <strong>K±</strong>, which differs in exactly two ways. First, one person — <strong>Owen</strong>, who is there in both futures — drops from " +
      K_BASE[0].w +
      " to " +
      K_BOTH[1].w +
      ". Second, <strong>Nadia is added at " +
      K_WOND[1].w +
      "</strong>: the same wonderful life you were asked about earlier. Nobody else is touched.",
  },
  {
    id: "plusVsBoth",
    kind: "pair",
    label: "The modest addition against the harm",
    ...
    when: function (a) {
      return a.neutral_mod === "none" && a.greedy === "left";
    },
    pops: [K_MOD_OWEN, K_BOTH],
    names: ["K+", "K±"],
    title: "Two ways to add Nadia.",
    body:
      "<strong>K+</strong> has the same 500 people as K, plus Nadia at " +
      K_MOD[1].w +
      ". <strong>K±</strong> has Owen — one of the people from K — down to " +
      K_BOTH[1].w +
      ", and Nadia added at " +
      K_WOND[1].w +
      " instead. <strong>K± has higher total welfare, but with Owen worse off.</strong>",
  },
  {
    id: "trans_gt",
    kind: "principle",
    label: "Transitivity of better-than",
  ...
  },
  {
    id: "trans_none",
    kind: "principle",
    label: "Transitivity of not-worse-than",
  ...
  },
  {
    id: "trans_eq",
    kind: "principle",
    label: "Transitivity of equal-goodness",
  ...
  },
  {
    ...
    opts: [
      [
        "A",
        "Yes \u2014 a verdict does not depend on what else is on offer",
        "yes",
      ],
      [
        "B",
  ...
  },
  {
    ...
    id: "menu",
    kind: "menu",
    label: "Choosing from three",
    pops: [A_POP, B_POP, Z_POP],
    names: ["A", "B", "Z"],
    totals: true,
  ...
  },
  {
    ...
    id: "menu_alpha",
    kind: "principle",
    label: "Best of three, best of two",
    when: function (a) {
      return !!alphaCandidate(a);
    },
    ...
    opts: [
      [
        "A",
        "Yes \u2014 removing an option cannot dethrone what was already best",
        "yes",
      ],
      [
        "B",
        "No \u2014 which option is best can depend on the whole set offered",
        "no",
      ],
    ],
  },
...
function compile(ans) {
  ...
  function edge(type, a, b, sup) {
    E.push({ type: type, a: a, b: b, sup: sup });
  }
  function fromPair(qid, A, B, sup) {
    var v = ans[qid];
    if (!v || v === "abstain") return;
...
function closeUp(edges, useGt, useEq, allowed, eqTags) {
  ...
  function supOK(list) {
    ...
    for (var i = 0; i < list.length; i++)
      if (!allowed.has(list[i])) return false;
    return true;
  }
  function setOK(s) {
    if (!allowed) return true;
  ...
  edges.forEach(function (e) {
    ...
    if (e.type === "gt") {
      ...
      var n1 = new Set(e.sup);
      if (better(ne[a][b], n1)) ne[a][b] = n1;
      var n2 = new Set(e.sup);
      if (better(ne[b][a], n2)) ne[b][a] = n2;
    } else {
      var s1 = new Set(e.sup);
      if (better(eq[a][b], s1)) eq[a][b] = s1;
      var s2 = new Set(e.sup);
      if (better(eq[b][a], s2)) eq[b][a] = s2;
    }
  });
      ...
      for (i = 0; i < N; i++) {
        ...
        for (j = 0; j < N; j++) {
          ...
          if (useEq && eq[i][k] && eq[k][j]) {
            ...
            if (setOK(c) && better(eq[i][j], c)) {
              eq[i][j] = c;
              dirty = true;
            }
          }
          if (useGt && useEq && eq[i][k] && gt[k][j]) {
            c = unite(eq[i][k], gt[k][j], eqTags);
            c.add("trans_gt");
            if (setOK(c) && better(gt[i][j], c)) {
              gt[i][j] = c;
              dirty = true;
...
function findClashes(M) {
  ...
  for (var i = 0; i < M.N; i++) {
    ...
    for (var j = 0; j < M.N; j++) {
      if (i === j) continue;
      if (i < j && M.gt[i][j] && M.gt[j][i])
        emit("cycle", i, j, unite(M.gt[i][j], M.gt[j][i]));
      if (M.gt[i][j] && M.eq[i][j])
        emit("gt_and_eq", i, j, unite(M.gt[i][j], M.eq[i][j]));
      ...
      if (M.ne[i][j] && M.gt[i][j])
        emit("denied", i, j, unite(M.ne[i][j], M.gt[i][j]));
      if (i < j && M.ne[i][j] && M.eq[i][j])
        emit("denied", i, j, unite(M.ne[i][j], M.eq[i][j]));
    }
  }
  return out;
}
...
function hasClash(ans, allowedSet) {
  ...
  var useGt =
    ans.trans_gt === "yes" && (!allowedSet || allowedSet.has("trans_gt"));
  ...
  var eqTags =
    ans.menu_eq === undefined ? ["trans_eq"] : ["trans_eq", "menu_eq"];
  var useEq =
    ans.trans_eq === "yes" &&
    ans.menu_eq !== "no" &&
    (!allowedSet ||
      eqTags.every(function (t) {
        return allowedSet.has(t);
      }));
  var res = findClashes(
    closeUp(compile(ans), useGt, useEq, allowedSet || null, eqTags),
  );
  _memo.c[mk] = res;
  return res;
}
    ...
    (function rec(start, pick) {
      if (pick.length === size) {
        ...
        for (var f = 0; f < found.length; f++) {
          var sub = true;
          found[f].forEach(function (x) {
            if (!S.has(x)) sub = false;
          });
          if (sub) return;
        }
...
function eqChainMatters(ans) {
  ...
  var E = compile(ans),
    useGt = ans.trans_gt === "yes";
  var on = closeUp(E, useGt, true, null),
    off = closeUp(E, useGt, false, null);
  for (var i = 0; i < on.N; i++)
    for (var j = 0; j < on.N; j++) {
...
var EXTRA_CHECKS = [
  ...
  // rather than on the betterness ordering.
  {
    id: "alpha",
    run: function (a) {
      return a.menu_alpha === "yes" ? alphaCandidate(a) : null;
    },
  },
  ...
  // scored, so an unasked or rejected principle cannot bite.
  {
    id: "collapse",
    run: function (a) {
      return a.collapse === "yes" ? collapseCandidate(a) : null;
    },
  },
...
Object.keys(CODES).forEach(function (k) {
  DECODES[k] = {};
  Object.keys(CODES[k]).forEach(function (v) {
    DECODES[k][CODES[k][v]] = v;
  });
});
...
function syncHash() {
  ...
  try {
    history.replaceState(null, "", baseURL() + frag);
  } catch (e) {
    location.replace(baseURL() + frag);
  }
}
function readHash() {
  ...
  h.split("&").forEach(function (kv) {
    ...
    try {
      out[kv.slice(0, i)] = decodeURIComponent(raw);
    } catch (e) {
      out[kv.slice(0, i)] = raw;
    }
  });
...
function missingActive() {
  return activeIdx().filter(function (i) {
    return ANS[QUESTIONS[i].id] === undefined;
  });
}
...
function popSVG(popsList, names, opts) {
  ...
  var blocks = popsList.map(function (p, i) {
    var cnt = p.reduce(function (a, g) {
      return a + g.n;
    }, 0);
    var tot = p.reduce(function (a, g) {
      return a + g.n * g.w;
    }, 0);
    var l1 = cnt.toLocaleString() + " people",
      l2 = "total " + tot.toLocaleString();
    return {
      p: p,
      name: names[i],
      l1: l1,
  ...
  var tallest = 0,
    deepest = 0;
  popsList.forEach(function (p) {
    p.forEach(function (g) {
      tallest = Math.max(tallest, g.w);
      deepest = Math.max(deepest, -g.w);
    });
  });
  ...
  pad = Math.max(
    pad,
    blocks[blocks.length - 1].half - blocks[blocks.length - 1].w / 2 + 10,
  );
  ...
  var s =
    ...
    Math.round(VH) +
    '" role="img" aria-label="' +
    names.join(" compared with ") +
    (opts.totals ? ", with total welfare drawn to scale" : "") +
    '">';
  ...
  blocks.forEach(function (bl) {
    ...
    function side(g) {
      return g.side === "left" ? 0 : g.tag ? 2 : 1;
    }
    bl.p
      .slice()
      .sort(function (a, b) {
      ...
      .forEach(function (g) {
        ...
        s +=
          ...
          (gw - 2).toFixed(1) +
          '" height="' +
          Math.max(h, 2).toFixed(1) +
          '" fill="' +
          (pos ? "var(--blue)" : "var(--red)") +
          '" opacity="' +
          (pos ? ".85" : ".9") +
          (g.tag ? '" stroke="var(--ink)" stroke-width="1.5' : "") +
          '"/>';
        s +=
          '<text x="' +
          cx.toFixed(1) +
          '" y="' +
          (pos ? y - 10 : baseY + h + 20).toFixed(1) +
          '" text-anchor="middle" font-family="Space Mono, monospace" font-size="' +
          FS.val +
          '" fill="var(--ink)">' +
          g.w +
          "</text>";
        if (g.tag) {
          ...
          s +=
            ...
            cx.toFixed(1) +
            '" y1="' +
            (pos ? baseY + 4 : baseY + h + 26).toFixed(1) +
            '" x2="' +
            cx.toFixed(1) +
            '" y2="' +
            (ty - 13) +
    ...
    s +=
      ...
      '" y="' +
      nameY +
      '" text-anchor="middle" font-family="Fraunces, Georgia, serif" font-weight="900" font-size="' +
      FS.name +
      '" fill="var(--ink)">' +
      bl.name +
      "</text>";
  ...
  if (opts.totals) {
    ...
    var maxT = blocks.reduce(function (a, b) {
    ...
    blocks.forEach(function (bl, i) {
      ...
      s +=
        ...
        '" width="' +
        len.toFixed(1) +
        '" height="16" fill="var(--blue)" opacity=".85"/>';
      s +=
        '<text x="' +
        (mL + labW + len + 10).toFixed(1) +
        '" y="' +
        (yy + 5) +
...
function chainSVG(withArc) {
  ...
  CHAIN.forEach(function (st, i) {
    nodes.push({
      n: st.to.n,
      w: st.to.w,
      label: i === CHAIN.length - 1 ? "Z" : i === 0 ? "B" : "",
    });
  });
  // Same width curve as the pair diagrams, scaled down to fill one row.
  var raw = nodes.map(function (nd) {
    return wpx(nd.n);
  });
  var k =
    (FRAME - pad * 2 - gap * (nodes.length - 1)) /
    raw.reduce(function (a, b) {
      return a + b;
    }, 0);
  var cw = raw.map(function (r) {
    return r * k;
  });
  ...
  var content =
    pad * 2 +
    cw.reduce(function (a, b) {
      return a + b;
    }, 0) +
    gap * (nodes.length - 1);
  var W = Math.max(content, FRAME),
    VH = baseY + (withArc ? 200 : 104);
  var x = (W - content) / 2 + pad,
    xs = [];
  ...
  s +=
    ...
    '" text-anchor="middle" font-family="Space Mono, monospace" font-size="' +
    FS.cnote +
    '" letter-spacing=".11em" fill="var(--blue)">EACH RUNG: EVERYONE GAINS &#183; GOOD LIVES ARE ADDED &#183; TOTAL WELFARE UP</text>';
  ...
  if (withArc) {
    var x0 = xs[xs.length - 1],
      ...
      dip = baseY + 172;
    s +=
      '<path class="arc" style="--len:' +
      Math.round(Math.abs(x0 - x1) * 1.7) +
      '" d="M ' +
      x0.toFixed(1) +
      " " +
      top +
    ...
    s +=
      ...
      " " +
      top +
      ' l -5 9 l 10 0 z" fill="var(--red)"/>';
    s +=
      '<rect x="' +
      ((x0 + x1) / 2 - 160).toFixed(1) +
      '" y="' +
      (dip - 13) +
      '" width="320" height="26" fill="var(--panel)"/>';
    s +=
      '<text x="' +
      ((x0 + x1) / 2).toFixed(1) +
      '" y="' +
      ...
      '" text-anchor="middle" font-family="Space Mono, monospace" font-size="' +
      FS.cnum +
      '" letter-spacing=".1em" fill="var(--red)">AND YET A IS BETTER THAN Z</text>';
  }
  return s + "</svg>";
}
...
function renderQ() {
  ...
  var q = QUESTIONS[IDX],
    ...
    pos = A.indexOf(IDX);
  $("#counter").textContent =
    "Question " + (pos + 1) + " of " + A.length + " \u00B7 " + q.label;
  var t = "";
  ...
  if (q.pops)
    fig =
      '<div class="figure">' +
      popSVG(q.pops, q.names, { totals: q.totals }) +
      "</div>";
  ...
  if (SHARED)
    html +=
      '<div class="shared">Shared answers &middot; someone else\u2019s run, shown as they gave it</div>';
  html +=
    '<div class="q"><h2 class="qtitle">' +
    q.title +
    "</h2>" +
    '<p class="qbody">' +
    (typeof q.body === "function" ? q.body(ANS) : q.body) +
    "</p>" +
    fig +
    '<p class="qbody" style="margin-top:20px"><strong>' +
    ask +
    '</strong></p><div class="opts">';
...
var LABELS = {
  pareto: function (a) {
    return {
      ...
      abstain:
        "No view on whether the same people all being better off makes a future better.",
    }[a];
  },
  same_number: function (a) {
    return (
      "A future of 100 people is " +
      {
        left: "better than",
        right: "worse than",
  ...
  misery: function (a) {
    return (
      "Adding Nadia with a life of suffering makes the outcome " +
      {
        left: "worse",
        right: "better",
        equal: "neither better nor worse",
        none: "incomparable",
      }[a] +
      "."
    );
  },
  neutral_mod: function (a) {
    return (
      "Adding Nadia with a modest good life makes the outcome " +
      {
        left: "worse",
        right: "better",
        equal: "neither better nor worse",
  ...
  neutral_wond: function (a) {
    return (
      "Adding Nadia with a wonderful life makes the outcome " +
      {
        left: "worse",
        right: "better",
        equal: "neither better nor worse",
  ...
  plusVsBoth: function (a) {
    return (
      ...
      " K± — Nadia added at " +
      K_MOD[1].w +
      " against Owen down by 35 and Nadia added at " +
      K_WOND[1].w +
      "."
    );
  },
  benign: function (a) {
    return (
      ...
      " A \u2014 everyone gains, and good new lives are added."
    );
  },
  nae: function (a) {
    return (
      ...
      " A+ \u2014 same headcount, more total, more average, fully equal."
    );
  },
  generalize: function (a) {
    return {
      yes: "Those two verdicts hold identically at every rung of the ladder.",
      no: "Somewhere on the ladder those two verdicts flip.",
      abstain: "No view on whether those two verdicts survive repetition.",
    }[a];
  },
  collapse: function (a) {
    return {
      ...
      abstain: "No view on whether unrankability collapses at its boundary.",
    }[a];
  },
  menu_eq: function (a) {
    return {
      ...
      no: "A third option can change how the first two compare.",
      abstain: "No view on whether verdicts survive a wider menu.",
    }[a];
  },
  menu_alpha: function (a) {
    return {
      ...
      abstain: "No view on whether the best of three stays best in a pair.",
    }[a];
  },
  trans_gt: function (a) {
    return {
      ...
      abstain: "No view on whether \u201Cbetter than\u201D is transitive.",
    }[a];
  },
  trans_none: function (a) {
    return {
      yes: "\u201CNot worse than\u201D is transitive.",
      no: "\u201CNot worse than\u201D is not always transitive.",
      abstain: "No view on whether \u201Cnot worse than\u201D is transitive.",
    }[a];
  },
  trans_eq: function (a) {
    return {
      ...
      abstain:
        "No view on whether \u201Cexactly as good as\u201D is transitive.",
    }[a];
  },
};
  ...
  return (
    {
      ...
      abstain: "No view offered on which of A, B and Z is best.",
    }[v] || ""
  );
}
...
function greedyPriced(needs) {
  return {
    ...
    because: function (a) {
      ...
      var pareto =
        "Pareto puts K++ above K±: the same 501 people, Owen better off by " +
        harm +
        " in one of them and nobody worse.";
      if (a.neutral_wond === "none") {
        var put = a.greedy === "right" ? "above K" : "exactly level with K";
        return (
          "You placed K and K++ beyond ranking: not better, not worse, not equal. But you put K± " +
          put +
          ", and " +
          pareto +
          " Taken together, those two say K++ is better than K, which is the pair you had just declined to rank. " +
          "If adding a good life is enough to outweigh a loss, then it must also be better than no change."
        );
      }
      if (a.greedy === "none") {
        var said =
          a.neutral_wond === "equal"
            ? "exactly as good as leaving her out, so K and K++ stand level"
            : "worse than leaving her out, so K stands above K++";
        return (
          "You judged the ranking of K and K± to be indeterminate. But you said adding Nadia at " +
          K_WOND[1].w +
          " is " +
          said +
          ", and " +
          pareto +
          " Between them those two give K better than K± — a ranking of the pair you had just declined to rank."
        );
      }
      var said =
        a.neutral_wond === "equal"
          ? "You said adding Nadia at " +
            K_WOND[1].w +
            " is exactly as good as leaving her out, so K and K++ stand level."
          : "You said adding Nadia at " +
            K_WOND[1].w +
            " makes the outcome worse, so K stands above K++.";
      ...
      return (
        ...
        " " +
        pareto +
        " Between them those two give K better than K±, and you put K± " +
        put +
        " instead. " +
        moral
      );
...
function plusVsBothPriced(needs) {
  return {
    ...
    because: function (a) {
      var harm = K_BASE[0].w - K_BOTH[1].w;
      var said =
        a.plusVsBoth === "equal" ? "exactly as good as K+" : "better than K+";
      return (
        "You ranked K better than K±, and you ranked K± " +
        said +
        ". Chain those two and K comes out definitively better than K+, which collides with what you said above about adding Nadia at " +
        K_MOD[1].w +
        "."
      );
    },
...
var STORIES = [
  ...
  },
  {
    ...
    needs: ["benign", "nae", "trans_gt", "AvB"],
    when: function (a) {
      return a.benign === "none" && a.AvB === "left" && a.nae === "right";
    },
    title: "The rungs rank the step you declined to rank.",
    because:
      "You judged A to be incomparable against A+: not better, not worse, not equal. But you judged A better than B, and B better than A+, and better-than chains \u2014 so A is better than A+, which is a verdict on the very pair you had just judged indeterminate.",
  },
  {
    needs: ["benign", "nae", "generalize", "trans_gt", "AvZ"],
    when: function (a) {
      return a.benign === "right" && a.nae === "right" && a.AvZ === "left";
    },
    title:
      "Each rung of the ladder is an improvement, but you end up worse than you started.",
    because:
      "This is Parfit's mere addition argument. You said A is better than B, and that you'd make the same judgment at each rung; transitivity chains them the whole way down; but at the end, you preferred A to Z. The relation \u201Cbetter than\u201D now runs in a circle.",
    chain: true,
  },
  {
    needs: ["misery", "neutral_mod", "pareto", "trans_eq", "menu_eq"],
    when: function (a) {
      return a.misery === "equal" && a.neutral_mod === "equal";
    },
    ...
    world:
      "If two carriers of the Tay-Sachs gene have a child, there's a one-in-four chance that the child develops normally for six months, then loses sight, hearing and movement, and dies by age four. Your answers say that conceiving that child makes the world no worse, and that conceiving a healthy one makes it no better.",
  ...
  },
  {
    ...
    when: function (a) {
      return a.misery === "equal" && a.neutral_wond === "equal";
    },
    title: "Nadia cannot be worth nothing whichever way her life goes.",
    because:
      "You said K is exactly as good as the world where Nadia exists in agony, and exactly as good as the world where she exists with a wonderful life. Transitivity of equal-goodness makes those two worlds exactly as good as each other, even though Nadia is enormously better off in one.",
    world:
      "If two carriers of the Tay-Sachs gene have a child, there's a one-in-four chance that the child develops normally for six months, then loses sight, hearing and movement, and dies by age four. Your answers say that conceiving that child makes the world no worse, and that conceiving a healthy one makes it no better.",
    group: "nadiaNeutralRange",
  },
  {
    ...
    when: function (a) {
      return a.neutral_mod === "equal" && a.neutral_wond === "equal";
    },
...
function worldNote(s) {
  return (
    '<div class="world"><div class="tag"><div class="world-heading">Concrete Problem</div></div><p>' +
    s +
    "</p></div>"
  );
}
...
function bullets() {
  ...
  if (ANS.trans_gt === "no")
    out.push({
      ...
      world:
        "<strong>You can be money-pumped.</strong> There are some three outcomes A, B, and C such that you'd pay $20 to move from A to B, another $20 to move from B to C, and then $20 to move from C back to A. Nothing has changed, except that you're out $60.",
    });
  ...
  if (ANS.menu_eq === "no") {
    ...
    out.push({
      t: "You denied that a verdict survives a wider menu.",
      claims: ["menu_eq"],
  ...
  if (ANS.menu_alpha === "no")
    out.push({
      ...
      world:
        "<strong>You can be money-pumped.</strong> Say you rank A > B in the presence of C, but B > A when considered alone. If C is removed, you'd pay $20 to move from A to B; then if C is re-introduced, you'd pay $20 to move back to A.",
    });

  if (ANS.trans_eq === "no")
    out.push({
      t: "You rejected transitivity of equal-goodness.",
      claims: ["trans_eq"],
  ...
  if (ANS.pareto === "no")
    out.push({
      t: "You rejected the Pareto principle.",
      claims: ["pareto"],
      b: "Denying that a world is better when the very same people are all better off in it is about as revisionary as population ethics gets. Almost every theory in the field takes this as a fixed point.",
    });
  if (ANS.AvZ === "right")
    out.push({
  ...
  if (ANS.generalize === "no")
    out.push({
      t: "You said the verdict flips somewhere on the ladder.",
      claims: ["generalize"],
      b: "That leaves you owing an account of <em>where</em>. Every rung is qualitatively identical to every other; if rung 4 is fine and rung 5 is not, something must distinguish them. Critical-level and lexical views are attempts to say what.",
    });
  ...
  if (ANS.misery === "right")
    out.push({
      t: "You counted a life of suffering as a gain.",
      claims: ["misery"],
      b: "When Nadia\u2019s life holds far more suffering than good \u2014 a life it would have been better for her never to have had \u2014 you judged the world better for containing it.",
    });
  ...
  if (ANS.misery === "equal" || ANS.misery === "none") {
    ...
    out.push({
      t: "Adding a life of misery did not make the world worse.",
      claims: ["misery"],
      b:
        "Nadia\u2019s life at K\u2212 is so miserable that it would have been better for her never to have lived. " +
        mtail +
  ...
  if (ANS.neutral_mod === "left" || ANS.neutral_wond === "left") {
    ...
    if (ANS.neutral_wond === "left") worseLefts.push("neutral_wond");
    out.push({
      t: "You said a life worth living makes the world worse by being lived.",
      claims: worseLefts,
      b: "This goes well past the Procreation Asymmetry, which claims only that creating a happy person is not <em>good</em>. You have said it is positively <em>bad</em>.",
      world:
        "Think of a couple who want a child, and would raise it well: you are committed to saying the child's birth is nonetheless bad for the world.",
    });
  }
  ...
  var NADIA = [
    ["misery", "a life of agony"],
    ["neutral_mod", "a modest good life"],
    ["neutral_wond", "a wonderful life"],
  ];
  var inv = [],
    invIds = {};
  for (var i = 0; i < NADIA.length; i++)
    for (var j = i + 1; j < NADIA.length; j++) {
      var ri = RANK[ANS[NADIA[i][0]]],
        rj = RANK[ANS[NADIA[j][0]]];
      if (ri === undefined || rj === undefined) continue;
      if (ri > rj) {
        inv.push("adding " + NADIA[i][1] + " above adding " + NADIA[j][1]);
        invIds[NADIA[i][0]] = 1;
        invIds[NADIA[j][0]] = 1;
      }
    }
  if (inv.length)
    out.push({
      t: "Nadia\u2019s life gets better and your verdict gets worse.",
      claims: Object.keys(invIds),
      b:
        "You ranked " +
        inv.join(", and ") +
        " \u2014 the same woman, the same 500 people around her, and the only difference is how well her life goes. According to your choices, the value of bringing someone into existence <em>falls</em> as their life improves.",
    });
  ...
  if (
    ANS.greedy === "none" &&
    (ANS.neutral_wond === "none" || ANS.neutral_wond === "equal")
  ) {
    out.push({
      t: "A neutral addition made a harm unrankable.",
      claims: ["neutral_wond", "greedy"],
      b:
        "Owen is one of the 500 people in K, and he's there in K± too, " +
        HARM +
        " points worse off. That is a loss to somebody who exists either way — not to a merely possible person. At the same time, you judged that adding Nadia's happy life alone did not constitute an improvement. This is Broome’s greediness objection. Your answer is not inconsistent, but the cost is that adding a new person \"swallows\" the harm to Owen – on this view, adding Nadia isn't good in isolation, but it causes the harm to Owen to stop being bad.",
    });
  }
  ...
  if (
    ANS.greedy === "right" &&
    ANS.neutral_wond === "none" &&
    ANS.pareto !== "yes"
  ) {
    out.push({
      t: "An addition you could not rank outweighed a harm you could.",
      claims: ["neutral_wond", "greedy"],
      b:
        "You said that adding Nadia at " +
        K_WOND[1].w +
        " was incomparable to leaving her out, and then ranked K± above K — which takes her arrival to justify making Owen worse off, when he would have existed either way. An addition cannot be too indeterminate to compare against nothing and determinate enough to outweigh a named loss. If her arrival really is worth that much, the earlier question had an answer: it was better.",
    });
  }
  if (ANS.greedy === "equal") {
    var geClaims = ["greedy"];
    if (ANS.neutral_mod === "none") geClaims.push("neutral_mod");
    if (ANS.neutral_wond === "none") geClaims.push("neutral_wond");
    out.push({
      t: "You priced Nadia’s life at exactly Owen’s loss.",
      claims: geClaims,
      b:
        "Two things separate K from K±: Owen loses " +
        HARM +
        " points, and Nadia arrives at " +
        K_WOND[1].w +
  ...
  if (ANS.plusVsBoth === "none" && ANS.pareto === "yes") {
    out.push({
      t: "You declined to rank K+ against K±, even though K± is a Pareto improvement from an anonymous perspective.",
      claims: ["pareto", "plusVsBoth"],
      b:
        "The two outcomes differ only in Owen and Nadia: Owen goes from " +
        K_BASE[0].w +
        " to " +
        K_BOTH[1].w +
        ", Nadia from " +
        ...
        " to " +
        K_WOND[1].w +
        ". No person is added or removed. From an impartial perspective, K± is a Pareto improvement over K+: the worst-off person's welfare increases from 7 to 20, and the best-off goes from 55 to 70. All that changes are the names.<br><br>" +
      ...
      world:
        "<strong>Policy choices are impossible.</strong> You can never choose a social policy in any realistic scenario, <strong>even if population size does not change</strong> — the only way you can express a preference is if one policy is a Pareto improvement over another.<br><br>" +
  ...
  if (ANS.benign === "left")
    out.push({
      t: "Everyone gains, good lives are added, and you called it worse.",
      claims: ["benign"],
      ...
      world:
        "Distributing malaria nets leaves recipients healthier and better off, and also means more children survive to adulthood, but with worse lives than the global average. Your answer holds that distributing malaria nets may therefore be a <em>bad</em> thing.",
    });

  if (ANS.nae !== "right")
    out.push({
      t: "You denied that levelling up improves things.",
      claims: ["nae"],
      b:
        "B holds the same " +
        CHAIN[0].to.n +
        " people as A+, with more welfare in total, more on average, and more equality. There is a consistent motive available \u2014 the better-off group does lose, falling from " +
        CHAIN[0].plus[0].w +
        " to " +
        CHAIN[0].to.w +
        ", while the worse-off rise from " +
        CHAIN[0].plus[1].w +
        " \u2014 so a view that weighs losses to the well-off heavily can resist it. But it sets you against totalism, averagism and egalitarianism in one move.",
    });
  ...
  PAIRQS.forEach(function (k) {
    if (ANS[k] === "none") noneIds.push(k);
  });
  var noneCount = noneIds.length;
  depletionProblemText =
    "<strong>Parfit's Depletion problem.</strong> A country can burn through its natural resource reserves, raising living standards for a century but ruining the climate for future generations; or it can conserve resources and protect the environment. The choice changes who meets whom and when children are conceived, so the two futures are populated by entirely different people. No particular person is harmed by choosing to deplete resources, but it still seems that a wrong has been committed. By your answers, depleting the environment's resources cannot be judged as wrong.";
  if (noneCount >= 3) {
    out.push({
      t:
        noneCount === PAIRQS.length
          ...
          : "You judged " +
            ...
            " pairs unrankable.",
      claims: noneIds,
      b: "You said that these pairs were not <em>equal</em>, but that no betterness relation holds either way. An ordering with widespread incomparability cannot give much practical guidance \u2014 it will stay silent on most of the choices you would want it to settle.",
      world: depletionProblemText,
    });
  } else if (
    ANS.AvZ === "left" &&
    ANS.benign === "none" &&
    ...
    // Only raise this bullet if we don't raise the one above, because
    // they're redundant. I think this is unreachable, but it's here just in
    // case.
    out.push({
      t: "You ranked the two ends of the ladder but not a single step of it.",
      claims: ["AvZ", "benign"],
      b: "When comparing A against Z — the two ends of the ladder — you judged A to be better. But at each rung along the ladder — an increase in population, and a small decrease in welfare — you deemed the outcomes incomparable.",
      world: depletionProblemText,
    });
  }
  ...
  var notBetter = function (v) {
    return v === "equal" || v === "none";
  };
  if (
    ANS.misery === "left" &&
    (notBetter(ANS.neutral_mod) || notBetter(ANS.neutral_wond))
  ) {
    ...
    var paClaims = ["misery"];
    if (notBetter(ANS.neutral_mod)) paClaims.push("neutral_mod");
    if (notBetter(ANS.neutral_wond)) paClaims.push("neutral_wond");
    out.push({
      t: "You hold the Procreation Asymmetry.",
      claims: paClaims,
      b:
        "Creating a miserable life is bad; creating a happy one is not good. This matches most people's instincts, and it is not inconsistent on its own. It is, however, notoriously hard to ground: the obvious explanations of the first half tend to imply the opposite of the second." +
        route,
      world:
        "By the Procreation Asymmetry, human extinction need not be a bad thing: if future generations never come to exist, then no one has been harmed so long as the currently-alive generations are happy.",
    });
  }
  ...
  if (ANS.same_number === "none" || ANS.same_number === "equal") {
    ...
    out.push({
      t: "When the people change, a world with uniformly better-off people is not judged better.",
      claims: ["same_number"],
      b:
  ...
  if (ANS.same_number === "right" && duckedDiffN.length) {
    out.push({
      t: "Comparable when the numbers match, unrankable when they do not.",
      claims: ["same_number"].concat(duckedDiffN),
      b:
        "You determinately ranked two futures when they each had 100 people, while leaving " +
        (duckedDiffN.length === 1
          ? "one comparison"
          : duckedDiffN.length + " comparisons") +
...
function profile() {
  ...
  if (
    ...
    return "Your answers sit closest to a <strong>person-affecting view</strong> with an asymmetry: creating a bad life is bad, but creating a good life is neutral.";
  if (
    (ANS.neutral_mod === "equal" || ANS.neutral_mod === "none") &&
    (ANS.neutral_wond === "equal" || ANS.neutral_wond === "none") &&
...
var CARD_HTML = {
  alpha: function (al, n) {
    ...
    h +=
      ...
      n +
      " &middot; contraction inconsistency</div>";
    h +=
      '<h3 style="margin-top:10px">Adding a third option changed your mind about the first two.</h3>';
    ...
    h +=
      '<ol class="claims"><li>Offered ' +
      al.pairWinner +
      " and " +
      al.picked +
      " alone, you judged " +
      al.pairWinner +
      " the better of the two.</li>";
    h +=
      "<li>Offered A, B and Z together, you ranked " +
      al.picked +
      (al.several ? " as not worse." : " at the top.") +
      "</li>";
    h += "<li>" + claimText("menu_alpha") + "</li></ol>";
    h +=
      '<p class="because">This violates Sen\u2019s property &alpha;: if something is best in a set, it must still be best in any subset that contains it. ' +
      al.third +
      "\u2019s presence cannot " +
      (al.several
        ? "put " + al.picked + " alongside "
        : "make " + al.picked + " beat ") +
      al.pairWinner +
  ...
  collapse: function (cp, n) {
    ...
    h +=
      '<div class="hit"><div class="tag">Conflict ' +
      n +
      " &middot; collapsing principle</div>";
    h +=
      '<h3 style="margin-top:10px">Your unrankable case has a determinate one right next to it.</h3>';
    h +=
      '<ol class="claims"><li>' +
      claimText(vg.id) +
      "</li><li>" +
      ...
      "</li></ol>";
    h +=
      '<p class="because">You judged ' +
      vg.world +
      " unrankable against K, while " +
      an.world +
      " \u2014 the same world with Nadia\u2019s life going " +
      (cp.dir === "up" ? "better" : "worse") +
      " \u2014 you ranked determinately. Between welfare " +
      Math.min(vg.w, an.w) +
      " and " +
      Math.max(vg.w, an.w) +
      " for her, then, lies a point where one unit of wellbeing " +
  ...
  zrank: function (zr, n) {
    ...
    h +=
      '<div class="hit"><div class="tag">Conflict ' +
      n +
      " &middot; chaining through the gap</div>";
    h +=
      '<h3 style="margin-top:10px">Declining to rank the rungs does not stop the ladder.</h3>';
    h += '<ol class="claims">';
    zr.ids.forEach(function (id) {
      h += "<li>" + claimText(id) + "</li>";
    });
    h += "</ol>";
    h +=
      '<p class="because">This is Parfit\u2019s mere addition argument, but run on <em>not worse than</em> instead of <em>better than</em>. Being unrankable, A+ is not worse than A. B is better than A+, so B is not worse than A+ either. You said both verdicts repeat at every rung, and you said not-worse-than chains, so it chains all ' +
      2 * CHAIN.length +
      " steps down the ladder: Z is not worse than A. But you also judged A better than Z, which is to say Z <em>is</em> worse than A. Declining to rank the rungs did not stop the contradiction, because an unrankable pair is still a pair where neither is worse.</p></div>";
    return h;
  },
  plusVsBoth: function (pv, n) {
    var h = "",
      said = pv.dir === "equal" ? "exactly as good as" : "better than";
    h +=
      '<div class="hit"><div class="tag">Conflict ' +
      n +
      " &middot; treating Owen and Nadia unequally</div>";
    h +=
      '<h3 style="margin-top:10px">You treated Owen\u2019s welfare as more important than Nadia\u2019s.</h3>';
    h +=
      '<ol class="claims"><li>' +
      claimText("pareto") +
      "</li><li>" +
      claimText("plusVsBoth") +
      "</li></ol>";
    h +=
      '<p class="because">You ranked K+ ' +
      said +
      " K\u00b1. The two outcomes differ only in Owen and Nadia: Owen's welfare goes from " +
      K_BASE[0].w +
      " to " +
      K_BOTH[1].w +
      ", Nadia's from " +
      ...
      " to " +
      K_WOND[1].w +
      ". No person is added or removed. From an anonymous perspective, K\u00b1 is a Pareto improvement over K+: the worst-off person\u2019s welfare increases from " +
      K_MOD[1].w +
      " to " +
      K_BOTH[1].w +
      ", and the best-off goes from " +
      K_BASE[0].w +
      " to " +
      K_WOND[1].w +
...
function showResults() {
  ...
  var nHits = R.sets.length + R.extras.length,
    cardNo = R.sets.length;
  var B = bullets();
  var h = "";
  h += '<div class="hero" style="padding:6vh 0 0">';
  if (SHARED)
    h +=
      '<div class="shared">Shared results &middot; someone else\u2019s answers, replayed from the link</div>';
  ...
  h +=
    '<p class="qbody" style="max-width:62ch">A <strong>conflict</strong> is a set of your answers that cannot all be true together. A <strong>bullet</strong> is a position of yours that is perfectly consistent, but implies a result that many find unpleasant or counterintuitive.</p>';

  if (nHits === 0) {
    h +=
      '<div class="clean"><div class="eyebrow" style="color:var(--blue)">Clean crossing</div><h3 style="margin-top:8px">Nothing you said conflicts with anything else you said.</h3><p class="qbody">That\'s rarer than you might think. But consistency in population ethics must be bought with counter-intuitive conclusions. The bullets you\'ve bitten are listed below.</p></div>';
  }
  ...
  R.sets.forEach(function (S, i) {
    ...
    h += '<ol class="claims">';
    QUESTIONS.forEach(function (q) {
      if (S.has(q.id)) h += "<li>" + claimText(q.id) + "</li>";
    });
    h += "</ol>";
    ...
    if (st && st.chain) {
      h += '<div class="chainwrap">' + chainSVG(true) + "</div>";
    }
    if (st && st.world) {
      h += worldNote(st.world);
    }
    h += "</div>";
  });
  ...
  if (B.length) {
    h +=
      '<hr class="rule thin" style="margin-top:44px"><div class="eyebrow">Bullets bitten</div>';
    B.forEach(function (b) {
      h +=
        '<div class="bullet"><div class="tag">Consistent, but costly</div><h3 style="margin-top:8px">' +
        b.t +
        "</h3>";
      // The answers that produced the bullet, shown the same way a conflict
      // shows the set behind it: one numbered claim per answer, in question
      // order so the list reads the same whichever bullet it sits under.
      if (b.claims && b.claims.length) {
        h += '<ol class="claims">';
        QUESTIONS.forEach(function (q) {
          if (b.claims.indexOf(q.id) >= 0)
            h += "<li>" + claimText(q.id) + "</li>";
        });
        h += "</ol>";
      }
      h +=
        ...
        b.b +
        "</p>" +
        (b.world ? worldNote(b.world) : "") +
        "</div>";
    });
  }

  h +=
    '<hr class="rule" style="margin-top:44px"><div class="eyebrow">Where you landed</div>';
  h += '<p class="qbody" style="font-size:19px">' + profile() + "</p>";

  h +=
    '<hr class="rule thin"><div class="eyebrow">Your answers in full</div><table class="rev">';
  ...
  QUESTIONS.forEach(function (q, i) {
    ...
    h +=
      ...
      revLabel(q.label, i) +
      "</td><td>" +
      (q.id === "menu" ? menuClaimText(ANS.menu) : claimText(q.id)) +
      "</td></tr>";
  });
  h += "</table>";

  h +=
    '<hr class="rule thin" style="margin-top:44px"><div class="eyebrow">Save or share</div>';
  h +=
    '<p class="qbody" style="font-size:17px">This link carries every answer you gave. Bookmark it to keep this page, or send it to someone and they will see exactly what you see.</p>';
  h +=
    '<div class="sharebox"><input id="sharelink" readonly spellcheck="false" value="' +
    shareURL().replace(/&/g, "&amp;").replace(/"/g, "&quot;") +
  ...
  h += SHARED
    ? "<span></span>"
    : '<button class="btn ghost" id="rback">&larr; Back</button>';
  h +=
    '<button class="btn ghost" id="again">' +
    (SHARED ? "Take the test yourself" : "Start over") +
    "</button></div>";
